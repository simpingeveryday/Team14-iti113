...
def api_recommend(city: str, min_guests: int = 2, max_price: float = 150.0,
    ...
    if only_saved:
        # Filtered here, AFTER the candidate set is formed and BEFORE scoring, so the
        # value term still takes its reference from the same pool as an unfiltered
        # query. Filtering `frame` instead would silently redefine what "global"
        # scaling is global over.
