...
probability. WHERE the min and max come from decides whether w_value does anything:
    ...
    global      fitted over every row in every city, BEFORE filtering. Prices span ten
                currencies, so the range is set by outliers elsewhere; a Paris
                candidate set then occupies a sliver of it, every listing scales to
                nearly the same number, and the ranking is instant_book_prob alone at
                any w_value below 1. This is what the notebook served.
    candidates  fitted over the listings actually being ranked.

The default is `global`, which reproduces the existing client output. `--scaling
candidates` is the other one. /recommend reports which it used and warns when w_value
turns out to have changed nothing, and the self test asserts both behaviours -- so
whichever is chosen, it is chosen rather than inherited.
"""
...
#
#   global      over every row in every city, fitted before any filtering. What the
#               notebook served, and what the existing client output reproduces.
#   candidates  over the listings actually being ranked, fitted after filtering.
#
# This is not a tuning knob, it decides whether w_value does anything at all. Prices
# span ten currencies, so the global range is set by outliers in Rio and Istanbul; a
# Paris candidate set then occupies a sliver of it, every listing scales to nearly the
# same number, and the ranking is instant_book_prob alone regardless of w_value.
RECOMMEND_SCALING = os.environ.get("RECOMMEND_SCALING", "global").lower()
...
def corpus_load(force: bool = False):
    global _CORPUS, _CORPUS_ERROR, _CORPUS_VERSION
...
def corpus_status() -> str:
    ...
    base = (f"corpus unavailable ({where}): {_CORPUS_ERROR}" if df is None
            else f"{len(df):,} listings from {where}")
...
def _api_unreachable(e: Exception) -> str:
    return (f"The recommender API at {RECSYS_API_URL} did not answer "
            f"({e.__class__.__name__}: {e}). Run the FastAPI cell in "
            f"Test_API_Gradio.ipynb, or start this app with --recsys-backend inline.")
...
def recsys_recommend(city: str, min_guests: int = 2, max_price: float = 150.0,
                     w_value: float = 0.5, w_convenience: float = 0.5) -> dict:
    ...
    out["backend"] = "inline"
    if api_error:
        out["backend_note"] = (f"Ranked in this process: the API at {RECSYS_API_URL} "
                               f"was unreachable ({api_error}).")
    return out
...
def _value_score(frame, candidates):
    ...
    if RECOMMEND_SCALING == "global":
        ...
        lo, hi = float(np.nanmin(deal)), float(np.nanmax(deal))
...
def api_recommend(city: str, min_guests: int = 2, max_price: float = 150.0,
    ...
    if candidates.empty:
        return {"results": [], "count": 0,
                "message": f"No listings match your criteria in {city}."}
    candidates["scaled_value_score"] = _value_score(frame, candidates)
...
def _fastapi_app():
    ...
    def _recommend(q: SearchQuery):
        return api_recommend(q.city, q.min_guests, q.max_price, q.w_value, q.w_convenience)
...
def guest_recommend(city, min_guests, max_price, w_value, w_convenience):
    """POST /recommend, rendered."""
    out = recsys_recommend(city, min_guests, max_price, w_value, w_convenience)
    if not out.get("results"):
        return ([], out.get("message") or f"Nothing in {city} matches those filters. "
                                          f"Raise the budget or lower the guest count.",
...
def guest_body(host_state=None) -> None:
    ...
    g_search.click(guest_recommend, [g_city, g_guests, g_budget, g_wv, g_wc],
...
def cmd_recommend(city: str, min_guests: int, max_price: float, w_value: float,
                  w_convenience: float, as_json: bool) -> int:
